...
- Lighthouse budgets (100/100/100/100 on `/`, ≥95 per study) are specified in the plan but not wired into CI, and `@lhci/cli` is **no longer installed** (removed 2026-09-14, #55). It sat installed and unwired — no lighthouserc, no CI job, no script — while carrying two advisories on `extract-zip` 2.0.1, a package with no patched release at any version, whose second advisory red-lit the dependency gate and blocked a critical Next RCE fix. Removing it cleared 279 packages and took this lockfile to zero vulnerabilities. Check budgets manually until the gate lands; wiring it (#28) means `npm i -D @lhci/cli` plus a `lighthouserc`, and the extract-zip advisories return with it — weigh them then, against the gate's actual value.
